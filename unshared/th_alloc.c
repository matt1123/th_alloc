 ...
 *
 *
 * Worked on by Matthew Krause and Sean Xiao
 * Original work under oath of UNC's Honor Pledge.
 */
...
struct superblock_bookkeeping * alloc_super (int power) {
  ...
  page = mmap(&sb, SUPER_BLOCK_SIZE, PROT_READ|PROT_WRITE, MAP_ANONYMOUS|MAP_PRIVATE, -1,0);
  ...
  if (!pool->free_objects) {
    ...
    while (bkeep != NULL) {
      ...
      if(bkeep->free_count){
        ...
        if ((128/(1<<power)-1)-bkeep->free_count == 1) {
...
void free(void *ptr) {
  ...
  int sz = 1<<(bkeep->level+5);
  memset(ptr, FREE_POISON, sz);

  struct object * tmp = bkeep->free_list;
  struct object * ptr_obj = (struct object *) ptr;
  bkeep->free_list = ptr_obj;
  bkeep->free_list->next = tmp;
  bkeep->free_count++;
  levels[bkeep->level].free_objects++;
  int max_freecount = 128/(1<<bkeep->level)-1;
  if (bkeep->free_count==max_freecount){
   ...
   */
   // memset(ptr, FREE_POISON, sizeof(ptr_obj));
  ...
  while (levels[bkeep->level].whole_superblocks > RESERVE_SUPERBLOCK_THRESHOLD) {
    ...
    struct superblock_pool pool_start = levels[bkeep->level];
    struct superblock_bookkeeping *tmp_bk = pool_start.next;



    while (tmp_bk->next) {
      if (tmp_bk->next->free_count==max_freecount) {
        // tmp_bk->next = tmp_bk->next->next;
        struct superblock_bookkeeping *rm_sb = tmp_bk->next;
        tmp_bk->next = tmp_bk->next->next;
        munmap(rm_sb, SUPER_BLOCK_SIZE);
        levels[bkeep->level].whole_superblocks--;
      } else {
        tmp_bk = tmp_bk->next;
      }
